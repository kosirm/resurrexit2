...
class PyMuPDFSpanParser:
    ...
    def _extract_span_data(self, pdf_path: str) -> Dict:
            ...
            for block in text_dict["blocks"]:
                if "lines" in block:
                    for line in block["lines"]:
                        line_text = ''.join([fix_slovenian_encoding(span['text']) for span in line['spans']])
        ...
        # Combine consecutive quote mark lines
    ...
    def _merge_quote_marks_at_same_y(self, text_lines: List[Dict], y_pos: float) -> Dict:
        ...
        if quote_marks and text_content:
            ...
            # Assume each quote should be separated by about 8-10 characters worth of space
    ...
    def _is_title_line(self, text: str, font_size: float, is_pink: bool) -> bool:
        ...
        # For Slovenian songs, we'll be more lenient with color requirements
        return (is_mostly_uppercase and
                ...
                # is_pink and  # Pink color preferred but not required for Slovenian
...
def fix_slovenian_encoding(text):
    """Fix common Slovenian character encoding issues in PDFs"""
    if not text:
        return text

    # Common Slovenian character mappings from incorrect PDF encoding
    char_map = {
        'è': 'č',  # è -> č (most common issue)
        'È': 'Č',  # È -> Č
        'š': 'š',  # This should already be correct, but just in case
        'Š': 'Š',  # This should already be correct
        'ž': 'ž',  # This should already be correct
        'Ž': 'Ž',  # This should already be correct
        # Add more mappings if we discover other issues
    }
...
def sanitize_filename(title: str) -> str:
    ...
    title = fix_slovenian_encoding(title)
...
def main():
    ...
    # Always generate title-based filename for Slovenian songs
    output_file = f"{input_basename}.chordpro"  # Default fallback

    # Generate title-based filename for logging (if Slovenian file)
    if input_basename.startswith("Pesmarica"):
        ...
        # Extract title from ChordPro content for logging
        song_title = ""
        ...
        if song_title:
            ...
            if sanitized_title and number_prefix:
                ...
                output_file = f"{sanitized_title}.chordpro"

    # Write output
    if output_file:
        with open(output_file, 'w', encoding='utf-8') as f:
            f.write(chordpro_content)
        print(f"✅ ChordPro saved to: {output_file}")
    else:

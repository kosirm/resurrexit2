#!/usr/bin/env python3
"""
ChordPro to HTML Converter - Equal Line Spacing Version
Special version that maintains consistent line spacing regardless of chord presence.
Ideal for songs like DRUGA EUHARISTIJSKA MOLITVA where uniform spacing is important.
"""
...
class ChordProToHTMLArial:
    ...
    def convert_chordpro_to_html(self, chordpro_content: str) -> str:
        ...
        for line in lines:
            # Skip empty lines and metadata (but NOT inline comments)
            if not line.strip():
                continue

            # Handle inline comments (comments that appear in verse content)
            if line.strip().startswith('{comment:') and not self._is_metadata_comment(line.strip()):
                # This is an inline comment - render it as a comment line
                comment_content = line.strip()[9:].rstrip('}').strip()  # Remove {comment: and }
                html_lines.append('            <div class="verse-line">')
                html_lines.append('                <div class="role-column"></div>')  # Empty role
                html_lines.append('                <div class="lyric-column">')
                html_lines.append(f'                    <div class="comment inline-comment">{self._escape_html(comment_content)}</div>')
            ...
            if '\t' in line:
                ...
                # Convert ChordPro line to HTML with equal spacing
                chord_html, lyric_html = self._convert_chordpro_line_equal_spacing(lyric_part)
    ...
    def _convert_chordpro_line_arial(self, chordpro_line: str) -> Tuple[str, str]:
        ...
        if chord_positions:
            ...
            for pos, chord in chord_positions:
                ...
                pixel_pos = self.calculate_text_width(text_before)
    ...
    def _convert_chordpro_line_equal_spacing(self, chordpro_line: str) -> Tuple[str, str]:
        """Convert a ChordPro line to chord HTML and lyric HTML with equal spacing"""
        if '[' not in chordpro_line:
            return "", self._escape_html(chordpro_line)
        ...
        # Generate chord HTML with equal spacing (character-based positioning)
        chord_html = ""
        if chord_positions:
            chord_spans = []
            ...
            for i, (pos, chord) in enumerate(chord_positions):
                # Calculate character position (equal spacing)
                char_pos = pos
                ...
                if pos in position_offsets:
                    ...
                    char_pos = pos + position_offsets[pos]
                    # Update offset for next chord at this position
                    chord_width = len(chord)
                    position_offsets[pos] += chord_width + 1  # 1 character spacing between consecutive chords
                else:
                    # First chord at this position
                    chord_width = len(chord)
                    position_offsets[pos] = chord_width + 1  # Prepare offset for next chord

                # Use character-based positioning for equal spacing
                chord_spans.append(f'<span class="chord" style="left: {char_pos * 0.6}em;">{chord}</span>')
    ...
    def _is_metadata_comment(self, line: str) -> bool:
        """Check if a comment line is metadata (kapodaster) or inline comment"""
        comment_content = line[9:].rstrip('}').strip().lower()  # Remove {comment: and }
        # Metadata comments are kapodaster or appear at the very beginning/end
        return ('kapodaster' in comment_content or 'kapo' in comment_content)
    ...
    def _extract_comments_and_kapodaster(self, lines: List[str]) -> Tuple[str, List[str]]:
        ...
        while i < len(lines):
            ...
            if line.startswith('{comment:'):
                ...
                elif self._is_metadata_comment(line):
                    # This is a metadata comment (not inline)
...
<head>
    ...
    <style>
        ...
        .verse-line {{
            display: table-row;
            height: 32px;  /* Fixed height for equal spacing */
        }}
        .role-column {{
            display: table-cell;
            width: 40px;
            ...
            font-weight: bold;
            padding-right: 4px;
            padding-bottom: 4px;  /* Match the padding of lyric column */
            color: #333;
            font-size: 16px;  /* Match increased font size */
            height: 32px;  /* Match the fixed height */
        }}
        .lyric-column {{
            display: table-cell;
            vertical-align: bottom;  /* Align text to bottom for consistent baseline */
            position: relative;
            height: 32px;  /* Fixed height matching verse-line */
            padding-bottom: 4px;  /* Space for text baseline */
        }}
        .chord-line {{
            color: {self.pink_color};
            font-weight: bold;
        ...
        .chord {{
            position: absolute;
            top: 1px;  /* Position chords even closer to text (5px lower) */
            white-space: nowrap;
        }}
        .lyric-text {{
        ...
        .inline-comment {{
            color: {self.pink_color};
            font-style: italic;
            font-size: 16px;  /* Match verse text size */
            margin: 0;  /* No extra margin for inline comments */
            padding: 0;
        }}
    </style>
</head>
